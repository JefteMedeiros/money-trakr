...
>(({ className, children, variant = "glass", ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex h-9 w-full items-center dark:text-white justify-between whitespace-nowrap rounded-md border px-3 py-2 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
        getVariantClass(),
        className,
      )}
      {...props}
...
>(
  (
    { className, children, position = "popper", variant = "glass", ...props },
    ref,
  ) => {
    const getVariantClass = () => {
      if (variant === "default")
      ...
      const variants = {
        glass:
          "glass-bg backdrop-blur-[var(--blur)] border border-[var(--glass-border)] text-foreground shadow-[var(--glass-shadow)]",
        glassSubtle:
        ...
        fluted:
          "glass-fluted backdrop-blur-[var(--blur)] border border-[var(--glass-border)] text-foreground shadow-[var(--glass-shadow)]",
        crystal:
